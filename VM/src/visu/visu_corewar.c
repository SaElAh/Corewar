...
/*   Updated: 2020/02/25 19:28:41 by cghanime         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			print_dashboard(t_visu *visu)
{
	...
	mvwprintw(visu->dash, 34, 7, "Cycle to die : %4d",
	visu->cor->cycles_to_die);
...
void			print_winner(t_visu *visu, int ind_winner)
{
	...
	mvwprintw(visu->dash, 50, 7, "And the winner is ... Champion %s",
			visu->cor->champ[ind_winner].header.prog_name);
	wrefresh(visu->dash);
